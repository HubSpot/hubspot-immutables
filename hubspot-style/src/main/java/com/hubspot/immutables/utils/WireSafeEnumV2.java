...
public final class WireSafeEnumV2<T extends Enum<T>> {
  private static final ObjectMapper MAPPER = new ObjectMapper();
  private static final Map<Class<?>, Map<?, WireSafeEnumV2<?>>> ENUM_LOOKUP_CACHE = new CopyOnWriteMap<>();
  private static final Map<Class<?>, Map<String, WireSafeEnumV2<?>>> JSON_LOOKUP_CACHE = new CopyOnWriteMap<>();

  private final Class<T> enumType;
  private final String jsonValue;
  private final Optional<T> enumValue;

  private WireSafeEnumV2(Class<T> enumType, String jsonValue, T enumValue) {
  ...
  private WireSafeEnumV2(Class<T> enumType, String jsonValue) {
    this.enumType = checkNotNull(enumType, "enumType");
    this.jsonValue = checkNotNull(jsonValue, "jsonValue");
  ...
  public static <T extends Enum<T>> WireSafeEnumV2<T> of(@Nonnull T value) {
    checkNotNull(value, "value");

    Class<T> enumType = getRealEnumType((Class<T>) value.getClass());
    Map<?, WireSafeEnumV2<?>> enumLookupMap = ENUM_LOOKUP_CACHE.get(enumType);
    if (enumLookupMap == null) {
      ensureEnumCacheInitialized(enumType);
      enumLookupMap = ENUM_LOOKUP_CACHE.get(enumType);
    }
    return (WireSafeEnumV2<T>) enumLookupMap.get(value);
  }

  @Nonnull
  public static <T extends Enum<T>> WireSafeEnumV2<T> fromJson(
    @Nonnull Class<T> enumType,
    @Nonnull String jsonValue
  ) {
    return fromJson(enumType, jsonValue, WireSafeEnumV2::new);
  }

  @Nonnull
  @SuppressWarnings("unchecked")
  private static <T extends Enum<T>> WireSafeEnumV2<T> fromJson(
    @Nonnull Class<T> enumType,
    @Nonnull String jsonValue,
    @Nonnull BiFunction<Class<T>, String, WireSafeEnumV2<T>> fallback
  ) {
    ...
    Map<String, WireSafeEnumV2<?>> jsonLookupMap = JSON_LOOKUP_CACHE.get(enumType);
    if (jsonLookupMap == null) {
      ensureJsonCacheInitialized(enumType);
      jsonLookupMap = JSON_LOOKUP_CACHE.get(enumType);
    }
    WireSafeEnumV2<?> cached = jsonLookupMap.get(jsonValue);
    if (cached == null) {
      return fallback.apply(enumType, jsonValue);
    } else {
      return (WireSafeEnumV2<T>) cached;
    }
  }
  ...
  private IllegalStateException getInvalidValueException() {
    Collection<WireSafeEnumV2<?>> wiresafeEnumTypes = JSON_LOOKUP_CACHE
      .get(enumType)
      .values();
    String validMembers = Arrays.toString(
      wiresafeEnumTypes
        .stream()
        .map(WireSafeEnumV2::asString)
        .distinct()
        .sorted()
        .toArray()
    );
  ...
  public boolean equals(Object o) {
    ...
    WireSafeEnumV2<?> that = (WireSafeEnumV2<?>) o;
    return (
  ...
  private static <T extends Enum<T>> void initializeCache(Class<T> enumType) {
    ...
    Map<T, WireSafeEnumV2<?>> enumMap = new EnumMap<>(enumType);
    Map<String, WireSafeEnumV2<?>> jsonMap = new HashMap<>(
    ...
    for (int i = 0; i < enumConstants.length; i++) {
      ...
      WireSafeEnumV2<T> wireSafeEnum = new WireSafeEnumV2<>(
        enumType,
        jsonValue,
        enumValue
      );
  ...
  public static class Deserializer
    extends JsonDeserializer<WireSafeEnumV2<?>>
    implements ContextualDeserializer {
    private static final Map<JavaType, JsonDeserializer<WireSafeEnumV2<?>>> DESERIALIZER_CACHE = new ConcurrentHashMap<>();

    @Override
    public WireSafeEnumV2<?> deserialize(JsonParser p, DeserializationContext ctxt)
      throws IOException {
    ...
    private static <T extends Enum<T>> JsonDeserializer<WireSafeEnumV2<?>> newDeserializer(
      JavaType enumType
    ) {
      return new JsonDeserializer<WireSafeEnumV2<?>>() {

        @Override
        public WireSafeEnumV2<T> deserialize(JsonParser p, DeserializationContext ctxt)
          throws IOException {
          if (p.getCurrentToken() == JsonToken.VALUE_NULL) {
            ...
            return WireSafeEnumV2.fromJson(
              rawType,
              p.getText(),
  ...
  private static JavaType findWireSafeEnumType(
    ...
    throws JsonMappingException {
    if (contextualType == null || !contextualType.hasRawClass(WireSafeEnumV2.class)) {
      throw ctxt.mappingException("Can not handle contextualType: " + contextualType);
    } else {
      JavaType[] typeParameters = contextualType.findTypeParameters(WireSafeEnumV2.class);
  ...
  private static <T extends Enum<T>> WireSafeEnumV2<T> create(
    JavaType enumType,
    String jsonValue,
    JsonParser parser,
    DeserializationContext ctxt
    ...
    return maybeEnumValue
      .map(enumValue -> new WireSafeEnumV2<>(rawClass, jsonValue, enumValue))
      .orElseGet(() -> new WireSafeEnumV2<>(rawClass, jsonValue));
  }
