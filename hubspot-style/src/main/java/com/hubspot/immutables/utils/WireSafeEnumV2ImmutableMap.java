...
public final class WireSafeEnumV2ImmutableMap<T extends Enum<T>> {
  private static final ObjectMapper MAPPER = new ObjectMapper();
  private static final Map<Class<?>, Map<?, WireSafeEnumV2ImmutableMap<?>>> ENUM_LOOKUP_CACHE = new CopyOnWriteMap<>();
  private static final Map<Class<?>, Map<String, WireSafeEnumV2ImmutableMap<?>>> JSON_LOOKUP_CACHE = new CopyOnWriteMap<>();

  private final Class<T> enumType;
  private final String jsonValue;
  private final Optional<T> enumValue;

  private WireSafeEnumV2ImmutableMap(Class<T> enumType, String jsonValue, T enumValue) {
  ...
  private WireSafeEnumV2ImmutableMap(Class<T> enumType, String jsonValue) {
    this.enumType = checkNotNull(enumType, "enumType");
    this.jsonValue = checkNotNull(jsonValue, "jsonValue");
  ...
  public static <T extends Enum<T>> WireSafeEnumV2ImmutableMap<T> of(@Nonnull T value) {
    checkNotNull(value, "value");

    Class<T> enumType = getRealEnumType((Class<T>) value.getClass());
    Map<?, WireSafeEnumV2ImmutableMap<?>> enumLookupMap = ENUM_LOOKUP_CACHE.get(enumType);
    if (enumLookupMap == null) {
      ensureEnumCacheInitialized(enumType);
      enumLookupMap = ENUM_LOOKUP_CACHE.get(enumType);
    }
    return (WireSafeEnumV2ImmutableMap<T>) enumLookupMap.get(value);
  }

  @Nonnull
  public static <T extends Enum<T>> WireSafeEnumV2ImmutableMap<T> fromJson(
    @Nonnull Class<T> enumType,
    @Nonnull String jsonValue
  ) {
    return fromJson(enumType, jsonValue, WireSafeEnumV2ImmutableMap::new);
  }

  @Nonnull
  @SuppressWarnings("unchecked")
  private static <T extends Enum<T>> WireSafeEnumV2ImmutableMap<T> fromJson(
    @Nonnull Class<T> enumType,
    @Nonnull String jsonValue,
    @Nonnull BiFunction<Class<T>, String, WireSafeEnumV2ImmutableMap<T>> fallback
  ) {
    ...
    Map<String, WireSafeEnumV2ImmutableMap<?>> jsonLookupMap = JSON_LOOKUP_CACHE.get(
      enumType
    );
    ...
    WireSafeEnumV2ImmutableMap<?> cached = jsonLookupMap.get(jsonValue);
    if (cached == null) {
      return fallback.apply(enumType, jsonValue);
    } else {
      return (WireSafeEnumV2ImmutableMap<T>) cached;
    }
  }
  ...
  private IllegalStateException getInvalidValueException() {
    Collection<WireSafeEnumV2ImmutableMap<?>> wiresafeEnumTypes = JSON_LOOKUP_CACHE
      .get(enumType)
      .values();
    String validMembers = Arrays.toString(
      wiresafeEnumTypes
        .stream()
        .map(WireSafeEnumV2ImmutableMap::asString)
        .distinct()
        .sorted()
        .toArray()
    );
  ...
  public boolean equals(Object o) {
    ...
    WireSafeEnumV2ImmutableMap<?> that = (WireSafeEnumV2ImmutableMap<?>) o;
    return (
  ...
  private static <T extends Enum<T>> void initializeCache(Class<T> enumType) {
    ...
    Map<T, WireSafeEnumV2ImmutableMap<?>> enumMap = new EnumMap<>(enumType);
    ImmutableMap.Builder<String, WireSafeEnumV2ImmutableMap<?>> jsonMap = ImmutableMap.builderWithExpectedSize(
    ...
    for (int i = 0; i < enumConstants.length; i++) {
      ...
      WireSafeEnumV2ImmutableMap<T> wireSafeEnum = new WireSafeEnumV2ImmutableMap<>(
        enumType,
        jsonValue,
        enumValue
      );
    ...
    JSON_LOOKUP_CACHE.put(enumType, jsonMap.build());
  }
  ...
  public static class Deserializer
    extends JsonDeserializer<WireSafeEnumV2ImmutableMap<?>>
    implements ContextualDeserializer {
    private static final Map<JavaType, JsonDeserializer<WireSafeEnumV2ImmutableMap<?>>> DESERIALIZER_CACHE = new ConcurrentHashMap<>();

    @Override
    public WireSafeEnumV2ImmutableMap<?> deserialize(
      JsonParser p,
    ...
    private static <T extends Enum<T>> JsonDeserializer<WireSafeEnumV2ImmutableMap<?>> newDeserializer(
      JavaType enumType
    ) {
      return new JsonDeserializer<WireSafeEnumV2ImmutableMap<?>>() {

        @Override
        public WireSafeEnumV2ImmutableMap<T> deserialize(
          JsonParser p,
          DeserializationContext ctxt
        )
          throws IOException {
          if (p.getCurrentToken() == JsonToken.VALUE_NULL) {
            ...
            return WireSafeEnumV2ImmutableMap.fromJson(
              rawType,
              p.getText(),
  ...
  private static JavaType findWireSafeEnumType(
    ...
    if (
      contextualType == null ||
      !contextualType.hasRawClass(WireSafeEnumV2ImmutableMap.class)
    ) {
      ...
      JavaType[] typeParameters = contextualType.findTypeParameters(
        WireSafeEnumV2ImmutableMap.class
      );
  ...
  private static <T extends Enum<T>> WireSafeEnumV2ImmutableMap<T> create(
    JavaType enumType,
    String jsonValue,
    JsonParser parser,
    DeserializationContext ctxt
    ...
    return maybeEnumValue
      .map(enumValue -> new WireSafeEnumV2ImmutableMap<>(rawClass, jsonValue, enumValue))
      .orElseGet(() -> new WireSafeEnumV2ImmutableMap<>(rawClass, jsonValue));
  }
